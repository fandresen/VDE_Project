...
const ModalEdit = () => {
    ...
    const sendData= async (data:dataformT)=>{
        try {
            const res = (await axios.post('/extract/add-candidate',{id:id,data}))
            if (res.status ==200) {
                dispatch(setAddCv())
  ...
  return (
    <>  
            <div className="w-[92vw] h-full absolute bg-[rgba(0,0,0,0.6)] top-0"onClick={handleDivClick}></div>
            ...
            <div className="w-[85vw] bg-white px-[2vw] py-[3vh] mx-auto rounded-2xl absolute bottom-[10vh]">
                <div className="relative">
                    ...
                    }
                    {
                        ...
                        <form action=""onSubmit={handleSubmit} ref={formulaire}>

                            <input type="email" placeholder="email" name="email" className={`w-full text-2xl border p-3 ${emailExist?'border-red-600':'border-black90'}`} value={email} readOnly/>
